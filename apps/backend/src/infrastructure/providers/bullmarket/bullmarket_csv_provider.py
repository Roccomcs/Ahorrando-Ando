"""
Provider para posiciones importadas desde CSV de Bull Market Brokers.

Credenciales:
  positions: list de dicts con {symbol, name, amount, price_usd}

Las posiciones se guardan encriptadas al momento del import.
Los precios se actualizan en tiempo real para criptos conocidas (CoinGecko).
Para acciones y bonos argentinos se usa el precio importado como referencia.
"""
...
class BullMarketCSVProvider(IFinancialProvider):
    ...
    @property
    def name(self) -> str:
        return "Bull Market (CSV import)"

    @property
    def provider_type(self) -> str:
        return "bullmarket_csv"
