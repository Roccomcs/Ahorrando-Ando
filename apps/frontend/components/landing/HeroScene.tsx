...
function Parallax({ children }: { children: React.ReactNode }) {
  ...
  return <group ref={group} data-size={size.width}>{children}</group>
}
...
function Scene() {
  ...
  useGSAP(() => {
    ...
    if (bitcoinRef.current) {
      ...
      tl.fromTo(bitcoinRef.current.position, { y: 5 }, { y: 2 }, 0)
    ...
    if (mpRef.current) {
      ...
      tl.fromTo(mpRef.current.position, { y: -5 }, { y: -2 }, 0)
    ...
    <Parallax>
      <group ref={bitcoinRef}>
        <CoinModel
          url={MODELS.bitcoin}
          position={[0, 2, 0]}
          metalBoost={{ metalness: 0.55, roughness: 0.35 }}
          floatSpeed={0.6}
          floatAmplitude={0.18}
          spinSpeed={0.25}
        />
      </group>
      <group ref={binanceRef}>
        <CoinModel
          url={MODELS.binance}
          position={[0, 0, 0]}
          metalBoost={{ metalness: 0.5, roughness: 0.38 }}
          floatSpeed={0.5}
          floatAmplitude={0.16}
          spinSpeed={-0.2}
          phase={2}
      ...
      <group ref={mpRef}>
        <CoinModel
          ...
          position={[0, -2, 0]}
          metalBoost={{ metalness: 0.4, roughness: 0.4 }}
          floatSpeed={0.55}
          floatAmplitude={0.2}
          spinSpeed={0.18}
          phase={4}
      ...
      <OrbitRing position={[0, 2, 0]} radius={1.5} color="#f7c268" speed={0.15} />
      <OrbitRing position={[0, 0, 0]} radius={1.5} color="#F0B90B" speed={-0.12} tilt={-0.35} />
      <OrbitRing position={[0, -2, 0]} radius={1.5} color="#41A4EF" speed={0.18} tilt={0.3} />

      <ambientLight intensity={0.5} />
      <directionalLight position={[4, 6, 5]} intensity={1.1} castShadow />
      <directionalLight position={[-4, -2, -3]} intensity={0.3} color="#3a6ea5" />
      <Environment resolution={64} environmentIntensity={0.7}>
        <Lightformer form="rect" color="#ffffff" intensity={2} position={[3, 3, 4]} scale={[3, 3, 1]} />
        <Lightformer form="rect" color="#41A4EF" intensity={1.2} position={[-4, -1, 3]} scale={[3, 4, 1]} />
        <Lightformer form="rect" color="#F0B90B" intensity={0.8} position={[0, -4, -2]} scale={[4, 2, 1]} />
      </Environment>
    </Parallax>
  )
}
